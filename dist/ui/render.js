...
function buildRadarSvg(statsHtml) {
    ...
    const gridPoints = (scale) => angles.map(a => `${cx + r * scale * Math.cos(a)},${cy + r * scale * Math.sin(a)}`).join(' ');
    const dataPoints = angles.map((a, i) => `${cx + r * pcts[i] * Math.cos(a)},${cy + r * pcts[i] * Math.sin(a)}`).join(' ');
    const gridLines = [0.25, 0.5, 0.75, 1].map(s => `<polygon points="${gridPoints(s)}" fill="none" stroke="currentColor" stroke-width="0.5" opacity="0.12"/>`).join('');
    const axisLines = angles.map(a => `<line x1="${cx}" y1="${cy}" x2="${cx + r * Math.cos(a)}" y2="${cy + r * Math.sin(a)}" stroke="currentColor" stroke-width="0.5" opacity="0.12"/>`).join('');
