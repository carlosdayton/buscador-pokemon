...
function buildRadarSvg(statsHtml) {
    ...
    const gridLines = [0.25, 0.5, 0.75, 1].map(s => `<polygon points="${gridPoints(s)}" fill="none" stroke="currentColor" stroke-width="0.5" opacity="0.2"/>`).join('');
    const axisLines = angles.map(a => `<line x1="${cx}" y1="${cy}" x2="${cx + r * Math.cos(a)}" y2="${cy + r * Math.sin(a)}" stroke="currentColor" stroke-width="0.5" opacity="0.2"/>`).join('');
